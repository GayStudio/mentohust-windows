...
void CProcess::startCert()
{
	...
	getSize();
	fillHeader();
	mainDlg->Output(_T("**请启动锐捷，如果锐捷正在运行，请耐心等待30秒**"));
...
BOOL CProcess::openAdapter()
{
	...
	sprintf(mac, "%02x:%02x:%02x:%02x:%02x:%02x", m_localMAC[0], m_localMAC[1], \
		...
		sprintf_s(buf, "(ether proto 0x888e and (ether dst %s or ether dst 01:80:c2:00:00:03 or ether src %s))", mac, mac);
	if (pcap_compile(m_hPcap, &fcode, buf, 1, 0xffffff) == -1//buf中更改过滤规则可以监听自己发出去的包ether proto 0x888e
...
/*根据发出去的心跳包得到key no*/
void CProcess::getEchoKeyFormEcho(const u_char *capBuf)
{
	CString strTemp;
	u_char cKeyNO[4],cNO[4];
	for (int i=0; i<4; i++){
		cKeyNO[i] = encode(capBuf[0x18+i]);
		cNO[i] = encode(capBuf[0x22+i]);
	}
	u_int32_t iKeyNO=*(u_int32_t *)cKeyNO;
	u_int32_t iNO=*(u_int32_t *)cNO;
//窗口输出(ntohl(iNO))//我不知道你的窗口输出函数
//别先处理以下代码，别先发包，先多捕捉几个验证以上代码的正确性。
	m_echoNo=ntohl(iNO);
	m_echoKey=ntohl(iKeyNO)-m_echoNo;

	strTemp.Format(_T("抓取心跳echoKey=%d echoNo=%d"),m_echoKey,m_echoNo);
...
static void pcap_handle(u_char *user, const struct pcap_pkthdr *h, const u_char *buf)
{
	...
	{
		/*
			截获锐捷第一个心跳包并把状态改为IDT_ECHO，下次本函数就不会运行了
		*/
		if (buf[0x0F]==0xbf&&memcmp(proc->m_localMAC, buf+6, 6)==0){
			/*
				如果锐捷多次认证成功，对key no进行校正  success=true
			*/
			if (proc->m_state==IDT_ECHO && success==true){//如果锐捷多次认证成功，对key no进行校正
		
				/**从心跳包抓取认证mac地址，这个可以任意时候切入锐捷的关键*/
				memcpy(proc->m_servMAC, buf, 6);
				strTemp.Format(LoadString(IDS_SERVMAC), buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
				mainDlg->Output(strTemp, 2);

				mainDlg->Output(_T("有可能是多次认证成功，不需结束锐捷，抓取心跳包key no："));
				proc->getEchoKeyFormEcho(buf);//校正心跳 key no
				/**从心跳包抓取认证mac地址，这个可以任意时候切入锐捷的关键*/
				success=false;
			}
			if(proc->m_state!=IDT_ECHO){//截获锐捷心跳包并把状态改为IDT_ECHO，下次这个if就不会运行了
				/**从心跳包抓取认证mac地址，这个可以任意时候切入锐捷的关键*/
				memcpy(proc->m_servMAC, buf, 6);
				strTemp.Format(LoadString(IDS_SERVMAC), buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
				mainDlg->Output(strTemp, 2);

				proc->getEchoKeyFormEcho(buf);//提取心跳包中的key no
				Sleep(1000);//睡眠1秒
				if(FindAndKillProcessByName(_T("8021x.exe"))){//立马结束掉锐捷
					mainDlg->Output(_T("******锐捷进程已经被强制结束******"));
				}

				Sleep(proc->m_echoTime);//延时心跳时间处理
				proc->switchState(IDT_ECHO);//改状态为echo
				mainDlg->ChangeTrayIcon();//改变图标认证成功状态
			}
		...
		{
			success=true;//认证成功标志，用于校正key no的
			...
			{
				proc->getEchoKey(buf);
				strTemp.Format(_T("认证成功echoKey=%d"),proc->m_echoKey);
				mainDlg->Output(strTemp);
			...
			if (proc->m_dhcpMode==1 || proc->m_dhcpMode==2)
				...
				//proc->switchState(IDT_ECHO);//这里不用了
			}
		}
		...
		else if (buf[0x0F]==0x00 && buf[0x12]==0x04)  /* 认证失败或被踢下线 */
		{
			if (proc->m_state==IDT_WAITECHO || proc->m_state==IDT_ECHO)
			{
				...
				mainDlg->MsgBox(LoadString(IDS_CERT_DOWN));
				proc->switchState(IDT_DISCONNECT);//状态重置为掉线状态
				//认证掉线
			}
...
static void printMsg(const u_char *buf, unsigned bufLen)
{
	...
	if ((length=0x1c+buf[0x1b]+0x69+39) < bufLen)
	{
		...
		char msg[257];
		strncpy_s(msg, serverMsg, length);
		msg[length] = '\0';
...
void CProcess::endCert()
{
	...
	if(m_state==IDT_ECHO){
		/*弄个对话框*/
		if (mainDlg->MsgBox(_T("认证断开注意：你可选择以下三种方式断开：\n\
			1，点击确定，请重启锐捷再次完成认证，然后断开认证，本程序将自动监听到锐捷断开事件，然后再点击本程序 退出。\n\
			2，点击确认，然后直接点击本程序 退出，将会直接停止心跳包发送，服务器会检测到没有继续心跳而导致自动下线。\n\
			3，点击取消，本程序将使用自带的没有v3校验的断开包发送给服务器，完成断开认证。然后再点击本程序 退出。\n\
			请根据学校情况自行选择，推荐使用 第1种、第2种方案，如果因为其中一种导致拉入黑名单，那么以后选择其他方式就是。\n\
			一般是不会因为下线拉入黑名单的。"), MB_OKCANCEL|MB_ICONWARNING) == IDOK){

			mainDlg->MsgBox(_T("你选择了确定，请按照第1种或者第2种方案执行下线"));
			mainDlg->GetDlgItem(IDOK)->EnableWindow(TRUE);
				
		}else{
			sendLogoffPacket();//发送自带的下线请求
...
{
	CString strTemp;
	strTemp.Format(_T("心跳中(重复发两次) echoKey=%d echoNo=%d"),m_echoKey,m_echoNo);
	mainDlg->Output(strTemp);
	if (m_bCernet)
	{
	...
	return pcap_sendpacket(m_hPcap, m_sendPacket, 0x2D);//free 0x2D=45 internet 0x3C=60
}
